// ============================================
// RENTA — Utility Functions
// ============================================

// Format number with commas (e.g., 3200 → 3,200)
...
export function formatPrice(amount) {
  return `${formatNumber(amount)} SAR`;
}
...
export const icons = {
  ...
  arrow_left: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m12 19-7-7 7-7"/><path d="M19 12H5"/></svg>`,
  arrow_right: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m12 5 7 7-7 7"/><path d="M5 12h14"/></svg>`,
  arrow_up: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m18 15-6-6-6 6"/></svg>`,
  ...
  x_circle: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="m15 9-6 6"/><path d="m9 9 6 6"/></svg>`,
  map_pin: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>`,
  ...
  external: `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" x2="21" y1="14" y2="3"/></svg>`,
  ...
  furnished: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 9V6a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v3"/><path d="M2 11v5a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-5a2 2 0 0 0-4 0v2H6v-2a2 2 0 0 0-4 0Z"/><path d="M4 18v2"/><path d="M20 18v2"/></svg>`,
};

// Feature icon mapping
export function getFeatureIcon(feature) {
  const lower = feature.toLowerCase();
  if (lower.includes('bedroom') || lower.includes('room') || lower.includes('غرف')) return '🛏️';
  if (lower.includes('living')) return '🛋️';
  if (lower.includes('kitchen') || lower.includes('مطبخ')) return '🍳';
  if (lower.includes('washroom') || lower.includes('bathroom') || lower.includes('حمام')) return '🚿';
  if (lower.includes('ac') || lower.includes('air')) return '❄️';
  if (lower.includes('courtyard') || lower.includes('garden') || lower.includes('حديقة')) return '🌿';
  if (lower.includes('parking') || lower.includes('garage')) return '🚗';
  if (lower.includes('pool') || lower.includes('مسبح')) return '🏊';
  if (lower.includes('wardrobe') || lower.includes('closet')) return '👔';
  if (lower.includes('balcony') || lower.includes('شرفة')) return '🏢';
  if (lower.includes('elevator')) return '🛗';
  if (lower.includes('cctv') || lower.includes('security')) return '📹';
  if (lower.includes('smart')) return '📱';
  if (lower.includes('maid') || lower.includes('driver')) return '🏠';
  if (lower.includes('dining')) return '🍽️';
  return '✨';
}

// Intersection Observer for scroll animations
export function observeElements(selector, className = 'animate-fade-in-up') {
